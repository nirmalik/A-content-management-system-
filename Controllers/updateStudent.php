...
if (($_POST['Sname'] !== "") && ($_POST['Sphone'] !== "") && ($_POST['Semail'] !== "")&& ($_POST['Courselist'] !== [])) {
    ...
    if (strlen($number) < 10 || strlen($number) > 10) {
        ...
        echo "fileErr";
          ...
          // if image uploaded check extensions and save to uploadedFile - save post values to class Student and update if form is validated - echo ok if update succeed.
        ...
        if (!empty($_FILES["file"]["type"])) {
            ...
            if ((($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg")) && in_array($file_extension, $valid_extensions)) {
                ...
                $updateStudent = updateStudent($newStudent);
                $updateStudentCourses = updateStudentCourses($newStudent, $coursesArray);

                if (($updateStudent) && ($updateStudentCourses)) {
                    echo 'ok';
                } else {
                    echo "err";
                }
